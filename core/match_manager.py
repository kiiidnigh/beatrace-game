...
class MatchManager:
    """
    Orchestriert die Spiellogik und Hintergrunddienste.
    """
    ...
    def __init__(self, game_state, network, router):
        ...
        # Initialisierung der Hintergrund-Dienste
    ...
    def cleanup(self):
        """Meldet alle Listener ab und beendet die Services sicher."""
        for event, func in self._listeners.items():
    ...
    def auto_start_turn(self):
        ...
        if next_player == self.game_state.my_name:
            ...
            EventBus.emit("UI_STATUS_UPDATE", data={"text": "Öffne DAW...", "color": "yellow"})
            ...
            EventBus.emit("UI_STATUS_UPDATE",
                          data={"text": f"Download fertig! Warte auf {next_player}...", "color": "gray"})

    def _on_daw_started(self, data):
        EventBus.emit("UX_START_TURN")
        EventBus.emit("UI_STATUS_UPDATE", data={"text": "Timer startet beim nächsten Klick!", "color": "yellow"})
    ...
    def _process_turn_end(self, data):
        ...
        EventBus.emit("UI_STATUS_UPDATE", data={"text": "Speichere Projekt... Bitte warten.", "color": "yellow"})
    ...
    def _on_sync_lock_timeout(self, data):
        EventBus.emit("UI_STATUS_UPDATE", data={"text": "Fehler beim Lesen der Datei!", "color": "red"})
    ...
    def _on_sync_lock_released(self, data):
        EventBus.emit("UI_STATUS_UPDATE", data={"text": "Prüfsumme berechnet...", "color": "yellow"})
        is_eliminated = self._is_time_up or (self.game_state.get_player_time(self.game_state.my_name) <= 0)
    ...
    def _start_download_watcher(self, data):
        if not data.get("expected_to_sync"):
            EventBus.emit("UI_STATUS_UPDATE",
                          data={"text": f"{data.get('last_player')} hat aufgegeben. Überspringe Download...",
                                "color": "orange"})
            threading.Timer(2.0, self.auto_start_turn).start()
            return

        EventBus.emit("UI_STATUS_UPDATE",
                      data={"text": f"{data.get('last_player')} ist fertig. Lade Projekt herunter...",
                            "color": "yellow"})
    ...
    def _on_download_finished(self, data):
        if data.get("success"):
            ...
            EventBus.emit("UI_STATUS_UPDATE", data={"text": "Download Timeout! Starte trotzdem...", "color": "red"})
            threading.Timer(2.0, self.auto_start_turn).start()
    ...
    def _on_time_up(self, data):
        if not self._is_time_up:
            ...
            EventBus.emit("UI_STATUS_UPDATE",
                          data={"text": "Hände weg! Auto-Save wird durchgeführt...", "color": "orange"})
